        ...
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Read responses aloud using device TTS.',
                style: TextStyle(fontSize: 12, color: context.textD),
              ),
              const SizedBox(height: 12),
              _card(
                context,
                child: Obx(() {
                  final tts = Get.find<KokoroTtsService>();
                  return Padding(
                        ...
                        SwitchListTile(
                          ...
                          subtitle: Text(
                            tts.ttsEnabled.value
                                ? 'Speaker icon shown on responses'
                                : 'Tap to enable voice playback',
                            style: TextStyle(
                              color: context.textD,
                              fontSize: 12,
                            ),
                          ...
                          secondary: Icon(
                            ...
                            color: tts.ttsEnabled.value
                                ? AppColors.accent
                                : context.textM,
                          ),
                          value: tts.ttsEnabled.value,
                          onChanged: (val) => tts.setEnabled(val),
                          activeThumbColor: AppColors.accent,
                          contentPadding: EdgeInsets.zero,
                        ),
                        if (tts.ttsEnabled.value) ...[
                          const SizedBox(height: 12),
                          SwitchListTile(
                            title: Text(
                              'Auto-play',
                            ...
                            secondary: Icon(
                              ...
                              color: tts.autoPlay.value
                                  ? AppColors.accent
                                  : context.textM,
                            ),
                            value: tts.autoPlay.value,
                            onChanged: (val) => tts.setAutoPlay(val),
                            activeThumbColor: AppColors.accent,
                            contentPadding: EdgeInsets.zero,
                          ),
                          const SizedBox(height: 12),
...
class _HardwareSettingsCardState extends State<_HardwareSettingsCard> {
  ...
  static Map<String, dynamic> _detectBestConfig() {
    ...
    if (Platform.isIOS) {
      // iOS: Metal GPU is always available on modern devices
      if (cores >= 6) {
        return {
          'backend': 'metal',
          'gpuLayers': 33,
          'reason': 'Metal GPU — best for iOS devices ($cores cores detected)',
        };
      }
      return {
        'backend': 'cpu',
        'gpuLayers': 0,
        'reason': 'CPU mode — safe for lower-end iOS ($cores cores)',
      };
    }

    if (Platform.isMacOS) {
      // macOS: Metal GPU is available on all Macs
      return {
        'backend': 'metal',
        'gpuLayers': 33,
        'reason': 'Metal GPU — hardware acceleration on Mac',
      };
    }

    if (Platform.isAndroid) {
      if (cores >= 8) {
        // High-end Android (e.g. Snapdragon 8 Gen 2+, Dimensity 9000+)
        return {
          'backend': 'opencl',
          'gpuLayers': 33,
          'reason':
              'OpenCL GPU — best for high-end SoC ($cores cores detected)',
        };
      } else if (cores >= 6) {
        // Mid-range Android
        return {
    ...
    // Windows/Linux: CPU is safest and most compatible
    return {
      'backend': 'cpu',
      'gpuLayers': 0,
  ...
  String get _currentConfigLabel {
    switch (_backend) {
      ...
      case 'metal':
        return 'GPU (Metal) • ${_gpuLayers.toInt()} layers';
      default:
        return 'CPU Only';
    }
  }
            ...
            Row(
              children: [
                ...
                if (Platform.isIOS || Platform.isMacOS)
                  _buildBackendButton('Metal', 'metal'),
                if (!Platform.isIOS && !Platform.isMacOS) ...[
