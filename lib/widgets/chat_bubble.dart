  ...
  Widget _buildContent(BuildContext context, bool isUser) {
    if (isUser) {
      return Padding(
        ...
        child: Text(
          message.content,
          style: TextStyle(fontSize: 15, color: context.text, height: 1.6),
        ),
      );
    }
      ...
      children: [
        ...
        if (message.content.isNotEmpty)
          Padding(
            ...
            child: Row(
              children: [
                // Speaker button (AI messages only, if TTS enabled)
                Obx(() {
                  final tts = Get.find<KokoroTtsService>();
                  if (!tts.ttsEnabled.value) return const SizedBox.shrink();
                  final isPlaying = tts.isSpeakingThis(message.content);
                  return InkWell(
                    onTap: () => tts.togglePlay(message.content),
                      ...
                      child: Row(
                        ...
                        children: [
                          Icon(
                            ...
                            size: 14,
                            color: isPlaying ? AppColors.accent : context.textD,
                          ),
