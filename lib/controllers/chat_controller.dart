  ...
  Future<void> sendMessage(String text, {String? modelFilename}) async {
    ...
    try {
      ...
      await for (final token in stream) {
        ...
        // Throttle UI refreshes — every 3rd token to reduce layout jank
        refreshCount++;
        if (refreshCount % 3 == 0) {
          chats.refresh();
        }
      ...
      // Auto-play response if TTS auto-play is enabled
      if (aiMsg.content.isNotEmpty && !aiMsg.content.startsWith('\u26a0')) {
        final tts = Get.find<KokoroTtsService>();
        if (tts.autoPlay.value) {
          tts.speak(aiMsg.content);
        }
      }
    }
  }
