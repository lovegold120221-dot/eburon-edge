...
class LlmService extends GetxService {
  ...
  Future<void> loadModel(String path) async {
    ...
    try {
      ...
      // Use smaller context on mobile to prevent OOM kills.
      // Desktop can handle 2048, but mobile devices with limited RAM
      // need 1024 to avoid the Low Memory Killer (LMK).
      final contextSize = Platform.isAndroid || Platform.isIOS ? 1024 : 2048;
      ...
      switch (storage.backendType) {
        ...
        case 'opencl':
          ...
          break;
        case 'metal':
          parsedBackend = GpuBackend.metal;
          break;
        default:
      ...
      // Optimize threads based on platform.
      // Desktop: use up to 8 threads for faster inference with smaller models.
      // Mobile: cap at 4 to avoid thermal throttling on mobile devices.
      final threadCount = Platform.isAndroid || Platform.isIOS
          ? (Platform.numberOfProcessors > 4 ? 4 : 0)
          : (Platform.numberOfProcessors > 4
                ? Platform.numberOfProcessors.clamp(2, 8)
                : 0);
      ...
      log?.info(
        'Backend=$parsedBackend, GPU layers=$userGpuLayers, ctx=$contextSize, threads=$threadCount',
        source: 'LLM',
      );
  ...
  /// Stop sequences are handled natively by the engine via GenerationParams.
  ...
  ///
  /// Optimization: passes GenerationParams to the native engine so stop
  /// sequences are handled at the C++ level, eliminating the old Dart-side
  /// buffer-and-scan overhead. Tokens are yielded immediately as they arrive
  /// for true per-token streaming.
  Stream<String> generate({
    ...
    // Minimal rolling window (last ~20 chars) for cross-token user-turn
    // detection. Stop sequences are handled natively by the engine.
    String lastChars = '';

    try {
      final prompt = _buildPrompt(messages, systemPrompt);

      // Let the engine handle stop sequences natively at the C++ level.
      // This eliminates the old Dart-side buffer-and-scan overhead,
      // yielding tokens immediately for true per-token streaming.
      ...
      await for (final token in _engine!.generate(prompt, params: genParams)) {
        ...
        // Check for user-turn hallucination across token boundaries
        lastChars = (lastChars + token);
        if (lastChars.length > 20) {
          lastChars = lastChars.substring(lastChars.length - 20);
        }
        if (_userTurnPattern.hasMatch(lastChars)) {
          break;
        }
